<script lang="ts">
	...
	type ThemeMaterialConfig =
		| (THREE.MeshStandardMaterialParameters & { angle?: number })
		| undefined;
	...
	type ModelTheme = {
		...
		type: string;
		structure: string;
		proportions: { shirt: number; pants: number };
		body: ThemeMaterialConfig;
		shirt?: ThemeMaterialConfig;
		pants?: ThemeMaterialConfig;
		eye: ThemeMaterialConfig;
		pupil: ThemeMaterialConfig;
		eyebrow: ThemeMaterialConfig;
	};

	const THEMES: Record<ThemeName, ModelTheme> = {
		spongebob: {
	...
	let currentThemeName: ThemeName = 'toon';
	let isMounted = false;
	...
	const CAMERA_POSITION = {
		x: 40, // 右侧距离，增大=更靠右
		y: TORSO_HEIGHT * 0.5 + 5, // 高度，增大=更高，减小=更低
		z: 40 // 前方距离，增大=更靠前
	};
	...
	const CAMERA_TARGET_OFFSET = {
		...
		y: BUST_Y_POS - 10, // 垂直偏移（相对于模型组中心），正值=向上，负值=向下
	...
	function _setupGeometry() {
		...
		modelGroup.position.set(MODEL_GROUP_POSITION.x, MODEL_GROUP_POSITION.y, MODEL_GROUP_POSITION.z);
	...
	function _updateEyeballGeometry(params: {
		mode: string;
		y: number;
		r_hemi: number;
		r_A: number;
	...
	function _setupMaterials(theme: ModelTheme) {
		for (const key of materialKeys) {
			const config = theme[key];
			if (config) {
				const materialProps = { ...config };
				delete materialProps.angle;
	...
	function isLightObject(obj: THREE.Object3D): obj is THREE.Light {
		return (obj as THREE.Light).isLight === true;
	}

	function _setupLighting(theme: ModelTheme) {
		scene.children.filter(isLightObject).forEach((light) => scene.remove(light));
		if (theme.type === 'toon') {
	...
	function _applyMaterials() {
		...
		(meshes.leftEyeball as THREE.Mesh).material = (meshes.rightEyeball as THREE.Mesh).material =
			materials.eye;
		(meshes.leftPupil as THREE.Mesh).material = (meshes.rightPupil as THREE.Mesh).material =
			materials.pupil;
		(meshes.leftEyebrow as THREE.Mesh).material = (meshes.rightEyebrow as THREE.Mesh).material =
			materials.eyebrow;
	}
	...
	export function setTheme(themeName: ThemeName, isInitial = false) {
